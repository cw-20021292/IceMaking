...
/*.. sean [24-12-26] 시간 표시 위해서 Wifi 헤더파일 추가..*/
...
void fnd_normal_state(void)
{
    ...
    {
		/*.. sean [24-12-26] wifi가 연결되었을 경우 시계 표시 ..*/
...
void fnd_water_out(void)
{
	/*.. 연속 선택 및 3초 버튼 누를수 있게 변경해서 fnd 추가해야함..*/
    ...
    {
        #if 0
        /*..hui [23-3-22오후 5:54:53] 마이컵 사용 설정하고 마이컵 추출시 컵 모양으로 변경..*/
...
void fnd_water_out_normal(void)
{
    ...
    switch( gu8_fnd_water_step )
    {
        ...
        case 1:
            ...
            #if 0
            /*..hui [23-2-9오후 12:28:00] ml 표시용.. 물양이슈로 %로 표시..*/
    ...
    if( bit_my_cup_setting_start == SET )
    {
        ...
        else
        {
            ...
            else
            {
                ...
                /*..hui [23-2-23오후 7:19:04] 마이컵 설정 중 FND에는 SEt 표시..*/
                mu8_temporary_thousand = DISPLAY_THOUSAND_OFF;
                mu8_temporary_hundred = DISPLAY_NUM_5;
...
void fnd_out( U8 mu8_direct, U8 mu8_thousand, U8 mu8_hundred, U8 mu8_ten, U8 mu8_one  )
{
	...
	/*..sean [25-02-10]fnd 휘도 조절하여 동작할 수 있도록 변경..*/
	if( F_dutyTest == CLEAR )
	{
