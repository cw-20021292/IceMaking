...
U8 low_water_led_flickering_timer(U8 flick_time, U8 interval)
{
    ...
    gu8_low_water_flick_tmr++;

    if(gu8_low_water_flick_tmr >= (flick_time - 1))
    {
        gu8_low_water_flick_tmr = 0;
        Bit4_Low_Water_LED_Flick = CLEAR;

    }
    else
    {
        if( (gu8_low_water_flick_tmr % interval) == 0 )
        {
            /*Bit4_Ice_Lock_Led1_White ^= SET;
                      Bit5_Ice_Lock_Led2_White ^= SET;*/
            u8flick_state = SET;
