...
U8 all_lock_flick_timer(U8 flick_time, U8 interval);
void start_low_water_flick(void);
U8 low_water_flick_timer(U8 flick_time, U8 interval);
...
U8 all_lock_flick_timer(U8 flick_time, U8 interval)
{
    ...
    if( gu8_all_lock_flick_cnt >= flick_time && u8_indi_flick_state == SET )
    {
        gu8_all_lock_flick_cnt = 0;
        gu16_all_lock_flick_timer = 0;
        u8_indi_flick_state = CLEAR;
...
U8 low_water_flick_timer(U8 flick_time, U8 interval)
{
    static U8 u8_low_water_flick_state = 0;

    if( gu16_low_Water_flick_timer ==  0 )
    {
        u8_low_water_flick_state = CLEAR;
    }
    else{}
    ...
    {
        u8_low_water_flick_state ^= SET;
        gu8_low_water_flick_cnt++;
    }
    else{}

    if( gu8_low_water_flick_cnt >= flick_time && u8_low_water_flick_state == SET )
    {
        gu8_low_water_flick_cnt = 0;
        gu16_low_Water_flick_timer = 0;
        u8_low_water_flick_state = CLEAR;
    ...
    return u8_low_water_flick_state;
}
