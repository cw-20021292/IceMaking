...
void water_select_return_decision(void)
{
    ...
    if(F_WaterOut_Change_State == SET)
    {
        ...
        /*..hui [17-12-14오후 4:31:12] 복귀 시간 7촐..*/
...
void room_water_init_check(void)
{
    /*..hui [24-1-3오후 1:33:20] 설치 플러싱할때는 탱크 완전배수까지 완전히 끝난후부터 가동..*/
    /*..hui [24-1-3오후 1:33:41] 온수탱크 에어때문에 바이메탈 단락 현상 연관있을 수 있음..*/
    /*..hui [24-1-3오후 1:34:13] 얼음,냉수,온수 전부 다 플러싱 종료되면 물 채우고 가동하도록..*/
    /*if( bit_install_flushing_state == SET )*/
    /*..hui [24-3-25오전 11:12:13] 일시적 플러싱 취소했을때도 정상 가동하도록....*/
    if( bit_install_flushing_state == SET && gu8_flushing_mode > FLUSHING_NONE_STATE )
    {
        F_WaterInit = CLEAR;
