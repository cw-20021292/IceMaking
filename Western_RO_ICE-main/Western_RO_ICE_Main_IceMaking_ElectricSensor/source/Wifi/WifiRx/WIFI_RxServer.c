...
U16 gu16_RAM_Monitor_RESP_SIZE;         // RAM DATA ÀÐ±â/¾²±â ¿äÃ»ÇÑ °¹¼ö (RAM address °¹¼ö)
...
static void RunAnalysis ( void )
{
    ...
    {
        if ( _STRNCMP_( (const char __FAR*) API_RX_A1011, (const char __FAR*) mai8RxAPINo, LENGTH_API_NO ) == N_TRUE ) // BLE : A1011 
        {  
            return;
        }
    }
    ...
    if ( _STRNCMP_( (const char __FAR*) API_RX_A9011, (const char __FAR*) mai8RxAPINo, LENGTH_API_NO ) == N_TRUE ) // BLE : A9011 
    {  
        return;
    }
#endif
    ...
    if ( GetUserInterface( USER_SMART_SENSING_STATUS ) == TRUE )
    {
        if ( _STRNCMP_( (const char __FAR*) API_RX_A1083, (const char __FAR*) mai8RxAPINo, LENGTH_API_NO ) == N_TRUE ) 
        {  
            return;
        }
    }
...
static U8 IsRecvPossible ( void )
{
    ...
    if ( GetUserInterface( USER_RX_STATUS ) != TRUE ) 
    {
        if ( IsExceptId( mu16ExceptId ) == TRUE ) 
        {
            ...
            {
                return FALSE; // Á¤¹ÐÁø´Ü ÁßÀÎ °æ¿ì Á¤¹ÐÁø´Ü Á¾·á Áö½Ã ÀÌ¿Ü ´Ù¸¥ ÀÔ·Â ±ÝÁö
            }
        }
    }
    else 
...
static U8 RxA7011 ( void )
{
    ...
    if ( _STRNCMP_( (const char __FAR*) mi8Id, (const char __FAR*) pBuf, LENGTH_WIFI_ID_ONE ) == N_TRUE ) // ID+:(5Byte) check
    {
        ...
        else // ASCII_NO_0 OR ASCII_NO_2 ?
        {
            ...
            SetWifiSendStatus(TX_DATA,WIFI_TX_SEND_BLE_CERT); // Á¡À¯ÀÎÁõ ½ÇÆÐ Àü¼Û (A7010)
        }
    }
...
static void DoFuncData ( I8* pBuf )
{
    ...
    #define SLEEP_COUNT         4  // ÃÑ ¿ë·® DATA °¹¼ö (Start H/M, END H/M)
    ...
    #define QUANTITY_COUNT      9  // ÃÑ ¿ë·® DATA °¹¼ö
    #define USE_MAX_QUANTITY    5   // ÃÖ´ë ¼³Á¤ °¡´ÉÇÑ ¿ë·® °¹¼ö
    #define USE_MIN_QUANTITY    2   // ÃÖ¼Ò ¼³Á¤ °¡´ÉÇÑ ¿ë·® °¹¼ö (¿¬¼ÓÆ÷ÇÔ)
            ...
            if ( _STRNCMP_( (const char __FAR*) mId, (const char __FAR*) mi8Id, LENGTH_WIFI_ID_ONE ) == N_TRUE ) // ID+:(5Byte) check
            {
                if ( pFun != NULL )
                {
                    ...
                    else if ((IsExceptSleepTimeID((pList+mi)->Id) == TRUE))
                    {
                        ...
                        if ((mi8Func[1] >= 0x30) && (mi8Func[1] <= 0x39)) // 2¹øÂ° ÀÚ¸´¼öÀÇ °ªÀÌ 0 ~ 9 ÀÎ °æ¿ì
                        {
                        ...
                        mu8Sleep_COUNT++;
                        if ((mu8FuncFinish == SET) || (mu8Sleep_COUNT == SLEEP_COUNT)) // ¿ë·® °¹¼ö Check
                        {
                            mu8Sleep_Finish = SET;
                        }
                    }
                    // ÃëÄ§½Ã°£¼³Á¤ ´ÜÀÏ Á¦¾î½Ã PASS

                    if ((mu8FuncFinish == SET) && (mu8Sleep_Finish == SET)) // (¿¹¿ÜÃ³¸® Ã¼Å© ¿Ï·á)
                    {
                        // ½½¸³½Ã°£ ¼³Á¤
                    ...
                    {
                        break;  // ¿ë·® ´ÜÀÏ Á¦¾î ½Ã PASS 
                    }
                    // LIST·Î ¹°·®¼³Á¤ ½Ã ¸Ç ¸¶Áö¸·¿¡ ´Ù½Ã Ã³¸® (µðÆúÆ® ¿ë·® ÀÓ½Ã¹öÆÛ ÀúÀå) (0066)
                    ...
                    // ¿ë·® ¿¹¿ÜÃ³¸® ---------------------------------(¿ë·®¼³Á¤¿Ï·á & µðÆúÆ®¼³Á¤¿Ï·á & µ¥ÀÌÅÍ¼ö½ÅÃ³¸® ¿Ï·á & ¿ë·® 9°³ ¼ö½Å ¿Ï·á)
                    if ((mQFinish == SET ) && (mQFinish_Defualt == SET ) && (mu8FuncFinish == SET) && (mu8QUANTITY_COUNT == QUANTITY_COUNT)) 
                    {
                        ...
                        for (mQi = 0; mQi < mu8QUANTITY_COUNT; mQi++)
                        {
                            ...
                            if (((mi8QUANTITY_SEL_Def-0x30) == mQi) && ((mi8QUANTITY_SEL_LIST_DATA[mQi]-0x30) != 0)) // À¯È¿ÇÑ µðÆúÆ® ¿ë·® ¼³Á¤ÀÎ°¡ ?
                            {
                                mQExceptionFinish_Defualt = SET;
                            }
                            else if ((mi8QUANTITY_SEL_Def-0x30) == 0x09)
                            {
                                mQExceptionFinish_Defualt = SET; // ¿¬¼ÓÀÎ°æ¿ì µðÆúÆ® ¼³Á¤¿Ï·á
                            }
                        }
                    ...
                    if ((mu8FuncFinish == SET) && (mu8QUANTITY_SEL_LIST_Def_IDNo != 0U) && (mQFinish == SET) && (mQExceptionFinish_Defualt == SET && (mQExceptionFinish_QUANTITY == SET))) // (¿¹¿ÜÃ³¸® Ã¼Å© ¿Ï·á)
                    {
