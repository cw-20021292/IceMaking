...
{   //  ID                  DefineTime          Count   CheckFunction           DoFunction
    ...
    {   CHK_SERVER_CNT,     WIFI_TIME_1SEC,     180,    CheckServerConnect,     ServerCheckReset},
    ...
    {   CHK_SEND_ACK,       WIFI_TIME_1SEC,     480,    CheckSendAck,           DoReset         }, // IND CHECK 180->480초 전에 연결끊김 후 [CHK_SERVER_CNT] 카운트 시작
...
static U8 CheckBleData ( void )
{
    if(GetWifiApStatus(STATUS_AP_BLE) == CLEAR) // BLE 비활성화
    {
...
static U8 CheckBleStatus2 ( void )
{
    if((GetWifiApStatus(STATUS_AP_BLE) != CLEAR) && (GetWifiApStatus(STATUS_AP_BLE_STATUS2_RESPONSE) != SET) ) // BLE 활성화가 아닌 경우
    {
        return TRUE;
    }
    return FALSE;
...
static void ServerCheckReset ( void )
{
    if(GetWifiApStatus(STATUS_AP_BLE_ALWAYS) != DONE) // ALWAYS SET 처리
    {
