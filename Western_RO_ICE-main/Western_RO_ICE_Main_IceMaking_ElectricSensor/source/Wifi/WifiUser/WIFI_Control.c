...
void WifiSendDataControl ( E_WIFI_DATA_T mu8Data, U8 mu8IdData )
{
    ...
    && ((mu8IdData != WIFI_FUNC_000B_WATER_SEL) && (mu8IdData != WIFI_FUNC_000D_WATER_OUTQUANTITY)))
    {
...
void IniEasyPairing ( void )
{
    ...
    if (mu8EepData != SET )
    {
        ...
        WifiRx.u8DisplayConnectCount = DONE; // BLE ¿⁄µø»∞º∫»≠ (√ ±‚∆‰æÓ∏µ)
    }
}
...
static void WifiApModekey ( void )
{
    ...
    ClearRxStatus();
    SetWifiApStatus(STATUS_AP_BLE_ALWAYS,CLEAR); // Soft AP KEY ¿‘∑¬ Ω√ º≠πˆø¨∞· Ω√ ªÛΩ√ON ªÛ≈¬ OFF
...
static void WifiApBLEkey ( void )
{
    ...
    { 
        SetWifiApStatus(STATUS_AP_BLE, CLEAR);
        SetWifiApStatus(STATUS_AP_BLE_ALWAYS,CLEAR); // BLE OFF KEY ¿‘∑¬ Ω√ º≠πˆø¨∞· Ω√ ªÛΩ√ON ªÛ≈¬ OFF
        if ( GetWifiServerStatus(STATUS_SVR_CONNECT) == SET )
        {
...
static void WifiOffKey ( void )
{
    ...
    // SetWifiBuzzSound(WIFI_BUZZER_CANCEL);
    ClearRxStatus();
    SetWifiApStatus(STATUS_AP_BLE_ALWAYS,CLEAR); // ªÛΩ√ON ªÛ≈¬ OFF
}
