...
#define            gu8_front_new_text_led                          U8FrontTempLEDB.byte
...
void led_control_mode_decesion(void)
{
    ...
    else if( bit_acid_clean_start == SET )
    {
        gu8_Led_Display_Step = LED_Display__ACID_CLEAN;
    }
    ...
    {
        /*..hui [23-10-25오전 9:21:56] 플러싱 중에도 시간 설정은 가능하게....*/
        gu8_Led_Display_Step = LED_Display__TIME_SETTING;
    }
    else if( gu8_flushing_mode > FLUSHING_NONE_STATE )
    {
    ...
    else if( F_All_Lock == SET && F_Child_Lock == SET )
    {
    ...
    else if( bit_sound_setting_start == SET )
    {
        gu8_Led_Display_Step = LED_Display__SOUND_SETTING;
    }
    else if( bit_memento_start == SET )
    {
        gu8_Led_Display_Step = LED_Display__MEMENTO;
    }
    else if( bit_ct_filter_type_start == SET )
    {
        gu8_Led_Display_Step = LED_Display__CT_FILTER_TYPE;
    }
    else if( F_WaterOut == SET )
    {
        gu8_Led_Display_Step = LED_Display__WATER_EXTRACT;
    }
    else if( F_Ice == SET && bit_ice_disp_pause == CLEAR )
    {
        /*..hui [23-5-11오후 2:32:39] 얼음, 물 동시 추출때문에 얼음 추출 화면을 우선순위 더 높게 xxxx..*/
        /*..hui [23-8-18오후 1:05:15] 온수 예열중에는 얼음 추출 화면 표시 안함..*/
        /*..hui [23-8-18오후 1:05:45] 단, 예열중 냉정 추출 끝나면 예열중이더라도 냉정으로 화면 남아있기 때문에..*/
        /*..hui [23-8-18오후 1:05:55] 그때만 예외적으로 얼음 추출 화면으로 전환..*/
        gu8_Led_Display_Step = LED_Display__ICE_EXTRACT;
    }
    else if( gu8_filter_alarm_popup_enable == SET )
    {
        gu8_Led_Display_Step = LED_Display__FILTER_ALARM;
    }
    else if( bit_display_sleep_start == SET || bit_9_hour_no_use_start == SET )
    {
        /*..hui [23-11-17오후 12:08:16] 슬립 모드가 온수세척, 설정보다 우선하도록....*/
        /*..hui [23-11-17오후 12:08:27] 슬립 모드중 온수세척 동작하면 슬립모드 표시 유지..*/
    ...
    /*else if( ( bit_uv_water_tank_out == SET ||  bit_uv_ice_tray_out == SET || bit_uv_ice_tank_out == SET )
             && gu8_uv_popup_enable == SET )*/
    else if( ( bit_uv_water_tank_out == SET ||  bit_uv_ice_tank_out == SET
               || ( bit_uv_ice_tray_out == SET && bit_ice_tray_making_enable == CLEAR ) )
             && gu8_uv_popup_enable == SET )
    {
        gu8_Led_Display_Step = LED_Display__UV;
    }
...
void on_all_control_led( void )
{
    gu8_front_temp_fnd_hundred = 0x7f;
